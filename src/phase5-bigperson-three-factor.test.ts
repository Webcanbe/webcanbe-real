...
describe("Phase 5 Bigperson mandatory three-factor boundary", () => {
  ...
  it("locks first Bigperson enrollment to Google + factor + verified passkey before role bootstrap", () => {
    ...
    expect(auth).toContain("credentialDeviceType")
    expect(auth).toContain("credentialBackedUp")
    expect(auth).toContain("device_type,backed_up")
    expect(auth).not.toContain('credentialDeviceType !== "singleDevice"')
    expect(auth).not.toContain('passkey.device_type) !== "singleDevice"')
    expect(auth).not.toContain("requires a device-bound passkey")
