...
describe("Phase 5 Gate 2 authenticated read smoke", () => {
  ...
  it("links Firebase identities to the current first-party account before provider login smoke", () => {
    ...
    expect(smoke).toContain("currentFirebaseProviderIds()")
    expect(smoke).toContain('providers.includes("github.com")')
    expect(smoke).toContain("currentFirebaseIdToken(true)")
    expect(smoke).toContain("await signOutFirebase().catch(()=>{})")
