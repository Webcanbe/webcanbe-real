...
it('normalizes Berry virtual edges only against operator-owned checksum and locator pins',()=>{
  const lock=parseYarnBerry(berry)
  const profile={packages:{
    'node_modules/react':{name:'react',version:'18.3.1',berryChecksum:checksum('1'),berryLocator:'react@npm:18.3.1'},
    'node_modules/wrapper':{name:'wrapper',version:'1.0.0',berryChecksum:checksum('2'),berryLocator:'wrapper@npm:1.0.0',dependencies:{peerful:'^2.0.0'}},
    'node_modules/wrapper/node_modules/peerful':{name:'peerful',version:'2.0.0',berryChecksum:checksum('3'),berryLocator:'peerful@virtual:abcdef12#npm:2.0.0',peerDependencies:{react:'^18.0.0'}},
  }}
  const normalized=normalizeAlternateLock(lock,{dependencies:{react:'^18.0.0',wrapper:'^1.0.0'}},profile)
  expect(normalized.packages['node_modules/wrapper/node_modules/peerful'].berryChecksum).toBe(checksum('3'))
  const sriOnly={packages:{...profile.packages,'node_modules/react':{name:'react',version:'18.3.1',integrity:'sha512-'+Buffer.alloc(64,1).toString('base64'),berryLocator:'react@npm:18.3.1'}}}
  expect(()=>normalizeAlternateLock(lock,{dependencies:{react:'^18.0.0',wrapper:'^1.0.0'}},sriOnly)).toThrow('checksum')
})
...
it('maps only the exact Yarn builtin compatibility patch to its underlying npm descriptor',()=>{
  ...
  const profile={packages:{'node_modules/typescript':{name:'typescript',version:'5.4.5',berryChecksum:checksum('4'),berryLocator:record.berryLocator}}}
  expect(normalizeAlternateLock(lock,{devDependencies:{typescript:'^5.4'}},profile).packages['node_modules/typescript'].version).toBe('5.4.5')
})
...
it('keeps npm tags exact and permits checksum omission only for unvisited conditional records',()=>{
  ...
  const profile={packages:{
    'node_modules/fsevents':{name:'fsevents',version:'2.3.3',berryChecksum:checksum('5'),berryLocator:'fsevents@npm:2.3.3',dependencies:{'node-gyp':'latest'}},
    'node_modules/fsevents/node_modules/node-gyp':{name:'node-gyp',version:'10.1.0',berryChecksum:checksum('6'),berryLocator:'node-gyp@npm:10.1.0'},
  }}
