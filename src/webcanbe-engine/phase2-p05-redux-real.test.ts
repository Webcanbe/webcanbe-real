...
maybe('parses the exact frozen reduxjs Yarn 4.2.2 lock without executing package-manager code',()=>{
  ...
  expect(createHash('sha256').update(source).digest('hex')).toBe('1d757201762d9525bed90f1ce55fed9f38ee99af895876674020d3cf87823abd')
  expect(createHash('sha256').update(fs.readFileSync(packagePath!)).digest('hex')).toBe('a3bd46f1fb94bdc5cf64855251c423ec90e80b304194026dbee8eaaa655cd9c8')
  ...
  expect(typescript.berryLocator).toBe('typescript@npm:5.4.5')
  const patched=lock.resolve('typescript','patch:typescript@npm%3A^5.4#optional!builtin<compat/typescript>')
  expect(patched.version).toBe('5.4.5')
  expect(patched.berryLocator).toContain('builtin<compat/typescript>')
