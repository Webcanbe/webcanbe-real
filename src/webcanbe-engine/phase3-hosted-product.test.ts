...
CREATE TABLE wcb_product_operators(user_id uuid PRIMARY KEY,active boolean NOT NULL DEFAULT true,epoch bigint NOT NULL DEFAULT 1,role text NOT NULL DEFAULT 'admin' CHECK(role IN ('reviewer','admin','bigperson')));
...
async function setup(faults: { afterMaterializationReservation?: () => Promise<void> } = {}, externalSource?: ExternalSourceProvider) {
  ...
  const product = new PostgresProductDomainStore(pool, access, source, faults, externalSource); await product.provisionOperator(operator.id, true, "bigperson")
...
describe("Phase 3 Admin Control backend", () => {
  ...
  it("allows only current operators to read Control state and makes revocation immediate", async () => {
    ...
    expect(await f.call(f.operator, "/control/operators/transition", { stepUpEvidenceId: stepUp.evidenceId, targetUserId: f.b.id, active: true, role: "admin", idempotencyKey: "grant-control-b" })).toMatchObject({ status: 200, body: { operator: { userId: f.b.id, active: true } } })
