...
it('P06 uses matching static Vite and referenced TS aliases in a nested source tree',async()=>{
 const f=fixture('apps/web','apps/web/src');f.write('vite.config.ts',`import path from 'node:path';export default {root:'apps/web',resolve:{alias:{'@':path.resolve(__dirname,'apps/web/src')}}}`);f.write('tsconfig.json',JSON.stringify({references:[{path:'./apps/web/tsconfig.json'}]}));f.write('apps/web/tsconfig.json',JSON.stringify({include:['src/main.tsx'],compilerOptions:{baseUrl:'.',paths:{'@/*':['./src/*']},verbatimModuleSyntax:true}}));f.write('apps/web/src/main.tsx',`import {createRoot} from 'react-dom/client';import App from '@/App';createRoot(document.getElementById('root')!).render(<App/>);`)
