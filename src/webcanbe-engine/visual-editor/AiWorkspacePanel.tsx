...
export default function AiWorkspacePanel({ open, onClose, connected, currentRevision, target, request, onApplied }: {
  ...
  return <section className="ai-workspace-panel" aria-label="AI source editor" onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); onClose() } }}>
    ...
    <div className="ai-panel-scroll">
      <div className="ai-chat-toolbar"><select aria-label="AI conversation"><option>New chat</option></select><button type="button" aria-label="Start new chat" onClick={()=>{controller.current?.abort();setPrompt("");setOutcome(undefined);setProposalDiff("");setError("");setPhase("idle")}}>+</button></div>
      <div className="ai-composer ai-prompt"><textarea ref={promptRef} aria-label="Ask Webcanbe" value={prompt} maxLength={4000} disabled={busy} placeholder="Ask Webcanbe…" onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") { event.preventDefault(); void generate() } }} /><footer><select aria-label="AI depth" value={mode} disabled={busy} onChange={event => setMode(event.target.value as AiMode)}><option value="standard">Standard · 1 Action</option><option value="deep">Deep · 3 Actions</option></select><button type="button" aria-label="Use selected element" title={target ? target.identity.file : "Select an element first"} aria-pressed={includeSelection && Boolean(target)} disabled={!target || busy} onClick={()=>setIncludeSelection(value=>!value)}>⌁</button><button type="button" aria-label="Toggle explain mode" aria-pressed={feature==="explain"} disabled={busy} onClick={()=>setFeature(value=>value==="modify"?"explain":"modify")}>+</button><button type="button" className="ai-send" aria-label="Generate proposal" disabled={!connected || !prompt.trim() || busy} onClick={() => void generate()}><span className="ai-visually-hidden">Generate proposal</span></button></footer></div>
