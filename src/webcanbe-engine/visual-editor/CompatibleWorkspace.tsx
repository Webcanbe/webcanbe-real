    ...
    <div className="compatible-layout">
      <aside className="compatible-files" aria-label="Project and source navigation">
        ...
        <div id="editor-project-panel" role="tabpanel" aria-label="Project" hidden={leftPanel !== "project"}>
          <aside className="project-settings-nav"><button className="project-settings-back" onClick={()=>setLeftPanel("pages")}><ArrowLeft/>Back to editor</button><small>Project settings</small><button className="active"><Settings2/>General</button><button onClick={()=>document.querySelector<HTMLElement>('.project-settings-runtime')?.scrollIntoView({behavior:'smooth'})}><Gauge/>Compatibility</button><button onClick={()=>openSurface("history")}><Code2/>Source history</button></aside>
          ...
          <div className="project-settings-fields">{hostedMode ? <>
            <label>Project{hostedProjects.length > 0 ? <select aria-label="Hosted project" value={projectId} onChange={event => { const id = event.target.value; window.history.replaceState({}, "", `/workspace/${id}`); setProjectId(id) }}>{hostedProjects.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <span>{project?.name || "Current working copy"}</span>}</label>
