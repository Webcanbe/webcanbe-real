...
export default function CompatibleWorkspace({agentChatEnabled=false}:{agentChatEnabled?:boolean}={}) {
  ...
  const previewCooldownRetries = useRef(0)
  ...
  const [viewport, setViewport] = useState<ViewportPreset>(() => typeof window !== "undefined" && window.innerWidth <= 680 ? "mobile" : "desktop")
  ...
  useEffect(() => {
    invalidateRaster(); previewController.current?.abort(); ++connectionEpoch.current
    ...
    return () => { cancelled = true; previewController.current?.abort(); ++connectionEpoch.current; window.removeEventListener("pagehide", pagehide); if (connected) stop(connected) }
  }, [projectId, accessKey, csrf, connectionAttempt])

  async function refreshPreview(incremental = false, manual = false) {
    ...
    if (!incremental || manual) previewCooldownRetries.current = 0
    selectionSource.current = "runtime"
    invalidateRaster(); setPreviewState("starting"); setPreviewStage(0); activeGeneration.current = ""; inspected.current = ""; ++inspectSequence.current
    setSelected(undefined); setHovered(undefined); setTarget(undefined)
    const slow = window.setTimeout(() => { if (current()) setPreviewStage(1) }, 7_000)
    const slower = window.setTimeout(() => { if (current()) setPreviewStage(2) }, 20_000)
    const timeout = window.setTimeout(() => controller.abort(), 45_000)
    let response
    try {
      response = await request("preview", { ...(incremental && preview?.transport === "raster" ? { command: "update", generation: preview.generation } : { route: routePath.current }), ...(expectedRevision ? { expectedRevision } : {}) }, controller.signal)
      if (response.ok === false && response.status !== 429 && incremental && preview?.transport !== "snapshot") response = await request("preview", { route: routePath.current, ...(expectedRevision ? { expectedRevision } : {}) }, controller.signal)
    }
    catch { if (current()) { setPreviewState("failed"); setMessage(controller.signal.aborted ? "Preview took too long. Retry to start a fresh render." : "Preview connection failed. Retry to start a fresh render.") } return }
    finally { window.clearTimeout(slow); window.clearTimeout(slower); window.clearTimeout(timeout); if (previewController.current === controller) previewController.current = null }
    if (!current()) return
    if (!response.ok && response.status === 429 && preview?.transport === "snapshot") {
      if (++previewCooldownRetries.current > 2) { setPreviewState("failed"); setMessage("Preview capacity is still busy. Retry in a moment."); return }
      setPreviewState("cooldown")
      setMessage(response.data.error ?? "Preview capacity is cooling down; retrying shortly.")
  ...
  return <><OnboardingStrip navigate={path => window.location.assign(path)}/><main className={`compatible-workspace${previewOnly ? " preview-only" : ""}${preview?.transport === "snapshot" && surface === "canvas" ? " full-page-canvas" : ""}`} data-source-revision={currentRevision}>
    ...
    <div className="compatible-layout">
      ...
      <section className={`compatible-preview-shell ${surface === "split" ? "split-mode" : ""}`}>
        ...
        <div hidden={surface !== "canvas" && surface !== "split"} ref={previewContainer} className={`compatible-frame-wrap ${viewport}`}>{!previewUrl && !preview && !session ? <div className="editor-canvas-empty"><b>Your canvas is ready</b><p>Connect or open a project to see its live page here.</p><button type="button" onClick={() => setLeftPanel("project")}>Project connection</button></div> : null}
        {session && previewState !== "ready" && previewState !== "stopped" && <div className="editor-preview-status" role="status" aria-live="polite"><div className="editor-preview-status-card"><span>Source preview</span><b>{previewState === "failed" || previewState === "expired" ? "Preview could not load" : previewState === "cooldown" ? "Preview capacity is busy" : previewStage >= 2 ? "Still rendering your project" : "Preparing your preview"}</b><p>{previewState === "failed" || previewState === "expired" ? message : previewState === "cooldown" ? "The managed renderer is cooling down. Webcanbe will retry twice, or you can retry now." : previewStage >= 2 ? "This is taking longer than usual. Your source is available in Code while rendering continues." : previewStage === 1 ? "The project runtime is starting. Your source is ready; you can keep working in Code." : "Loading the project’s real source, styles, and assets…"}</p>{(previewStage > 0 || previewState === "failed" || previewState === "expired" || previewState === "cooldown") && <button type="button" onClick={() => void refreshPreview(false, true)}>Retry preview</button>}</div></div>}
        <div className="editor-artboard" style={{width:viewportWidth*scale,height:frameHeight*scale}}><div className="preview-device" style={{...frameStyle, visibility: !previewUrl && !preview ? "hidden" : "visible"}}><div className="editor-artboard-label">{viewport === "mobile" ? "Phone" : viewport === "tablet" ? "Tablet" : "Desktop"} <span>{viewportWidth}</span></div><div className="editor-preview-viewport">{preview?.transport === "snapshot" ? <img key={preview.generation} src={preview.url} alt="Managed production project preview" draggable={false} onClick={selectSnapshot} style={{width:"100%",height:frameHeight,objectFit:"fill",display:"block",cursor:selectMode?"crosshair":"default"}} /> : <iframe key={preview?.generation ?? "unavailable"} ref={frame} referrerPolicy="no-referrer" onLoad={configurePreview} title="Running imported React/Vite project" src={preview?.html ? undefined : previewUrl || undefined} srcDoc={preview?.html ?? (previewUrl ? undefined : "<p>Preview unavailable. Source inspection remains available.</p>")} sandbox="allow-scripts" />}{!previewOnly && activeBox && activeBox.rect.width > 0 && activeBox.rect.height > 0 && <div className={`canvas-outline ${selected ? "selected" : ""}`} style={{ left: activeBox.rect.left, top: activeBox.rect.top, width: activeBox.rect.width, height: activeBox.rect.height }}>{selected && <span>{selected.tagName} · {selected.identity.file.replace("src/", "")}</span>}{selected && target && selectionSource.current === "runtime" && target.identity.elementStart === selected.identity.elementStart && target.identity.file === selected.identity.file && <SourceGestures key={JSON.stringify([projectId, preview?.generation, revision.current, selected.identity, selected.rect, viewport, authoringBreakpoint, effectScope, scale, routeInput, previewState])} target={target} origins={effectScope === "source" ? styleOrigins : target.repeated ? [] : styleOrigins.filter(origin => !origin.shared)} scale={scale} breakpoint={authoringBreakpoint} disabled={pending || !session || !selectMode || previewState !== "ready"} onEdit={edit => void mutate(edit)} />}</div>}</div></div></div></div>
      </section>
