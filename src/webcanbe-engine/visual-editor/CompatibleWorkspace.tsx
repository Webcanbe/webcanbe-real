...
import { MousePointer2, Hand, ChevronDown } from "lucide-react"
...
export default function CompatibleWorkspace() {
  ...
  function openSurface(value: "canvas" | "code" | "split" | "history") { if (value !== "canvas") setSourceUIOpened(true); setSurface(value); analytics.capture("wcb_editor_mode_changed", { source: "workspace", editor_mode: value === "canvas" ? "visual" : value }); const url = new URL(window.location.href); url.searchParams.set("mode", value); window.history.replaceState({}, "", url) }
  ...
  const frameStyle = { width: viewportWidth, height: 900, transform: `scale(${scale})` }
  ...
  return <><OnboardingStrip navigate={path => window.location.assign(path)}/><main className="compatible-workspace" data-source-revision={currentRevision}>
    ...
    <div className="compatible-layout">
      ...
      <section className={`compatible-preview-shell ${surface === "split" ? "split-mode" : ""}`}>
        ...
        {(surface === "canvas" || surface === "split") && <div className="editor-canvas-tools"><button aria-label="Select elements" aria-pressed={selectMode} onClick={()=>setSelectMode(true)}><MousePointer2/></button><button aria-label="Interact with preview" aria-pressed={!selectMode} disabled={preview?.transport==="snapshot"} onClick={()=>setSelectMode(false)}><Hand/></button><select aria-label="Canvas zoom" value={zoom} onChange={event=>setZoom(event.target.value==="fit"?"fit":Number(event.target.value))}><option value="fit">Fit · {Math.round(scale*100)}%</option><option value="0.5">50%</option><option value="0.75">75%</option><option value="1">100%</option><option value="1.25">125%</option></select><select aria-label="Canvas viewport" value={viewport} onChange={event=>setViewport(event.target.value as ViewportPreset)}><option value="desktop">Desktop</option><option value="tablet">Tablet</option><option value="mobile">Phone</option></select></div>}
        <div hidden={surface !== "canvas" && surface !== "split"} ref={previewContainer} className={`compatible-frame-wrap ${viewport}`}>{!previewUrl && !preview ? <div className="editor-canvas-empty"><b>{session ? "Preparing your preview" : "Your canvas is ready"}</b><p>{session ? "Source tools remain available while the preview starts." : "Connect or open a project to see its live page here."}</p><button type="button" onClick={() => setLeftPanel("project")}>Project connection</button></div> : null}<div className="editor-artboard" style={{width:viewportWidth*scale,height:900*scale}}><div className="preview-device" style={{...frameStyle, visibility: !previewUrl && !preview ? "hidden" : "visible"}}><div className="editor-artboard-label">{viewport === "mobile" ? "Phone" : viewport === "tablet" ? "Tablet" : "Desktop"} <span>{viewportWidth}</span></div><div className="editor-preview-viewport">{preview?.transport === "snapshot" ? <img key={preview.generation} src={preview.url} alt="Managed production project preview" draggable={false} onClick={selectSnapshot} style={{width:"100%",height:"900px",objectFit:"fill",display:"block",cursor:selectMode?"crosshair":"default"}} /> : <iframe key={preview?.generation ?? "unavailable"} ref={frame} referrerPolicy="no-referrer" onLoad={configurePreview} title="Running imported React/Vite project" src={preview?.html ? undefined : previewUrl || undefined} srcDoc={preview?.html ?? (previewUrl ? undefined : "<p>Preview unavailable. Source inspection remains available.</p>")} sandbox="allow-scripts" />}{activeBox && activeBox.rect.width > 0 && activeBox.rect.height > 0 && <div className={`canvas-outline ${selected ? "selected" : ""}`} style={{ left: activeBox.rect.left, top: activeBox.rect.top, width: activeBox.rect.width, height: activeBox.rect.height }}>{selected && <span>{selected.tagName} · {selected.identity.file.replace("src/", "")}</span>}{selected && target && selectionSource.current === "runtime" && target.identity.elementStart === selected.identity.elementStart && target.identity.file === selected.identity.file && <SourceGestures key={JSON.stringify([projectId, preview?.generation, revision.current, selected.identity, selected.rect, viewport, authoringBreakpoint, effectScope, scale, routeInput, previewState])} target={target} origins={effectScope === "source" ? styleOrigins : target.repeated ? [] : styleOrigins.filter(origin => !origin.shared)} scale={scale} breakpoint={authoringBreakpoint} disabled={pending || !session || !selectMode || previewState !== "ready"} onEdit={edit => void mutate(edit)} />}</div>}</div></div></div></div>
      </section>
