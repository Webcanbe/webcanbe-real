...
redux('round-trips every exact frozen Redux file, including its 2742928-byte Yarn release, and retains missing-graph refusal',async()=>{
  ...
  expect({profile:report.profile,issues:report.issues}).toEqual({profile:'react18-vite5-redux-msw-v1',issues:[]})
  expect(report.clientDependencies).toEqual(expect.arrayContaining(['@reduxjs/toolkit','@mswjs/data','msw','react','react-dom','react-router-dom']))
  expect(report.clientDependencies).not.toContain('react-redux')
  const trusted=JSON.parse(fs.readFileSync(path.join('runtime-profiles',report.profile,'package-lock.json'),'utf8'))
  const declared={...manifest.devDependencies,...manifest.dependencies},reachableRoots=report.clientDependencies.filter(name=>Object.prototype.hasOwnProperty.call(declared,name))
  expect(()=>normalizeAlternateLock(lock,manifest,trusted,reachableRoots)).not.toThrow()
  const missing=structuredClone(trusted);delete missing.packages['node_modules/@reduxjs/toolkit']
  expect(()=>normalizeAlternateLock(lock,manifest,missing,reachableRoots)).toThrow('Locked graph does not match pinned identity')
...
bun('preserves the retained Todo binary Bun lock as opaque bytes and refuses graph admission',async()=>{
  ...
  const p=project(root),report=inspectRuntime(p,process.cwd())
  expect(report.supported).toBe(false);expect(report.issues.some(i=>i.code==='unsupported-lockfile')).toBe(false);expect(report.issues.some(i=>i.code==='lock-conflict')).toBe(true)
  const copy=path.join(temp(),'project');await extractSafeZip(await exportProjectZip(p),copy)
