...
export class IncrementalPreviewCompiler {
  ...
  async build(key: string, fingerprint: string, options: BuildOptions): Promise<BuildResult> {
    ...
    if (entry?.fingerprint !== fingerprint) { await entry?.context.dispose(); this.contexts.delete(key); entry = undefined; this.transforms.clear() }
...
export type PreviewUpdateKind = "css-hot-update" | "incremental-rebuild-reload" | "generation-restart"
