...
const html = `<!doctype html><meta charset="utf-8"><title>Controlled preview viewer</title><style>html,body{margin:0;overflow:hidden;background:#fff}img{display:block;user-select:none;-webkit-user-drag:none}</style><img alt="Rendered project" draggable="false"><script>
let generation='',sequence=0,select=true;const img=document.querySelector('img');
addEventListener('message',event=>{
 if(event.source!==parent||!/^http:\\/\\/(localhost|127\\.0\\.0\\.1)(:\\d+)?$/.test(event.origin))return;
