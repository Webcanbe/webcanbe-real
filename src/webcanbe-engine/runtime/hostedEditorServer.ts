...
export function hostedEditorServer(editor: HostedEditor, tls: ServerOptions, distribution: string) {
  ...
  const server = createServer({ ...tls, minVersion: "TLSv1.2" }, (request, response) => {
    void (async () => {
      ...
      if (document) {
        response.writeHead(200,{"Content-Type":"text/html; charset=utf-8","Cache-Control":"no-store","X-Content-Type-Options":"nosniff","Referrer-Policy":"no-referrer","Content-Security-Policy":`default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-src ${editor.options.origins.viewerOrigin}; frame-ancestors 'none'; base-uri 'none'; form-action 'self'`})
