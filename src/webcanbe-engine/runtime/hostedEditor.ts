...
export class HostedEditor {
  ...
  constructor(readonly applicationRoot: string, readonly options: { pool: Pool; origins: HostedOriginPolicy; hosts: readonly HostedRunnerHost[]; identityProvider: IdentityProvider; allowSelfRegistration?: boolean; sessionLifetimeMs?: number; fastRefresh?: boolean; publicRuntimeValueProvider?: PublicRuntimeValueProvider; staticAssets?: TrustedStaticAssets; externalSourceProvider?: ExternalSourceProvider; managedSecrets?: ManagedPreviewSecrets; onError?: (error: unknown) => void }) {
    this.identity = new PostgresIdentityStore(options.pool, { allowSelfRegistration: options.allowSelfRegistration === true, sessionLifetimeMs: options.sessionLifetimeMs })
