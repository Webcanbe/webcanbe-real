...
export default function App() { const path = usePath(); if (path === "/") return <Landing/>; if (path === "/login") return <Auth/>; if (path === "/signup") return <Auth signup/>; if (path === "/browse") return <Browse/>; if (path.startsWith("/project/")) return <Detail reference={path.split("/").pop() ?? ""}/>; if (path.startsWith("/workspace/")) return <CompatibleWorkspace/>; if (path === "/projects") return <Projects/>; if (path === "/purchases") return <Purchases/>; if (path === "/dashboard") return <Dashboard/>; if (path === "/settings") return <Settings/>; if (path === "/plans") return <Plans/>; if (path === "/seller/projects/new") return <Seller page="new"/>; if (path === "/seller/projects") return <Seller page="projects"/>; if (path === "/seller") return <Seller/>; if (path === "/control") return <Control/>; return <Browse/> }
