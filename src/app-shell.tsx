...
export function useWorkspaces() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const next = productionAuthMode() ? await hostedProductClient.workspaces() : readLocal<string[]>('wcb-demo-workspaces', ['personal'])
        if (!current) return
  ...
  return { ids, error, loading, selected, select }
}
...
const icons = { overview: LayoutDashboard, projects: FolderKanban, marketplace: Store, purchases: ShoppingBag, creator: Sparkles, 'source-visual': PanelsTopLeft, 'source-code': FileCode2, 'source-split': Columns2, workspace: PanelsTopLeft, docs: BookOpen, settings: UserCircle2, account: Settings2, billing: CreditCard, notifications: Bell, help: CircleHelp }
export function ProductShell({ children, view, navigate, signOut }: { children: ReactNode; view: DashboardView; navigate: Navigate; signOut: () => Promise<void> }) {
  ...
  const item = (key: DashboardView) => { const Icon = icons[key]; return <AppLink key={key} to={appRoutes[key][0]} navigate={choose} className={'rd-nav-button' + (view === key ? ' active' : '')}><Icon/><span>{appRoutes[key][1]}</span></AppLink> }
  ...
  return <><OnboardingStrip navigate={navigate}/><div className={'rd-shell' + (sidebarOpen ? '' : ' rd-sidebar-collapsed')}>
    {sidebarOpen && <button className="wcb-sidebar-scrim" aria-label="Close navigation" onClick={() => setSidebarOpen(false)}/>}
    <aside className="rd-sidebar"><div className="rd-sidebar-header"><AppLink to="/dashboard" navigate={choose} className="wcb-shell-brand"><img src="/brand/webcanbe-mark.svg" alt=""/><span>WebCanBe</span></AppLink><div className="rd-menu-anchor"><button className="rd-team-switcher" aria-expanded={menu === 'team'} onClick={() => setMenu(menu === 'team' ? '' : 'team')}><span className="rd-team-logo"><PanelsTopLeft/></span><span className="rd-team-copy"><b>{workspaces.loading ? 'Loading workspace…' : workspaces.selected ? workspaceLabel(workspaces.selected, workspaces.ids.indexOf(workspaces.selected)) : 'Your workspace'}</b><small>Current workspace</small></span><ChevronsUpDown/></button>{menu === 'team' && <div className="rd-dropdown rd-team-dropdown"><span className="rd-dropdown-label">Workspaces</span>{workspaces.ids.map((id, index) => <button key={id} className="rd-dropdown-item" onClick={() => { workspaces.select(id); choose('/workspace') }}>{workspaceLabel(id, index)}{id === workspaces.selected && <Check/>}</button>)}{workspaces.error && <p role="alert">{workspaces.error}</p>}<button className="rd-dropdown-item" onClick={() => { setMenu(''); setCreate(true) }}><Plus/>Add workspace</button></div>}</div></div>
      <div className="rd-sidebar-content"><section className="rd-nav-group"><span className="rd-nav-label">General</span>{(['overview','projects','marketplace','purchases','creator'] as DashboardView[]).map(item)}</section><details className="rd-nav-group" open><summary>Source-backed editing<ChevronDown/></summary>{(['source-visual','source-code','source-split'] as DashboardView[]).map(item)}</details><section className="rd-nav-group"><span className="rd-nav-label">Pages</span>{item('workspace')}{item('docs')}</section><details className="rd-nav-group" open><summary>Settings<ChevronDown/></summary>{(['settings','account','billing','notifications'] as DashboardView[]).map(item)}</details>{item('help')}</div>
    ...
    </aside><div className="rd-content"><header className="rd-header"><button className="rd-sidebar-trigger" aria-label="Toggle sidebar" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(!sidebarOpen)}><PanelsTopLeft/></button><div className="rd-header-actions"><button className="rd-search" onClick={() => { setQuery(''); setSearch(true) }}><Search/><span>Search Webcanbe</span><kbd>⌘K</kbd></button><button className="rd-header-icon" aria-label="Notifications" onClick={() => choose('/notifications')}><Bell/></button><div className="rd-menu-anchor"><button className="rd-header-avatar" aria-label="Profile" aria-expanded={menu === 'profile'} onClick={() => setMenu(menu === 'profile' ? '' : 'profile')}><Avatar account={account}/></button>{menu === 'profile' && accountMenu(true)}</div></div></header>{children}</div>
