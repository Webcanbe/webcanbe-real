...
import { FolderKanban, Store, ShoppingBag, Sparkles, PanelsTopLeft, Columns2, FileCode2, BookOpen, Settings2, UserCircle2, CreditCard, Bell, CircleHelp, ChevronsUpDown, Plus, Search, X, LogOut, Check, ArrowRight, Grid2X2 } from 'lucide-react'
...
export function ProductPageHeader({ title, description, action, className = '' }: { title: string; description: string; action?: ReactNode; className?: string }) {
  return <header className={`wcb-product-page-header ${className}`.trim()}><div><h1>{title}</h1><p>{description}</p></div>{action && <div className="wcb-product-page-action">{action}</div>}</header>
}
...
export function ProductShell({ children, view, navigate, signOut }: { children: ReactNode; view: DashboardView; navigate: Navigate; signOut: () => Promise<void> }) {
  ...
  return <div className={'rd-shell' + (sidebarOpen ? '' : ' rd-sidebar-collapsed')}>
    ...
    </aside><div className="rd-content"><button className="rd-sidebar-trigger rd-floating-sidebar-trigger" inert={["settings", "account", "billing", "workspace"].includes(view)} aria-label="Toggle sidebar" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(!sidebarOpen)}><PanelsTopLeft/></button>{children}</div>
