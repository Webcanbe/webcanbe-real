...
function relativeImportCandidates(importer, specifier) {
  ...
  const directory = importer.split("/").slice(0, -1), parts = [...directory]
  for (const part of specifier.split("/")) {
    if (!part || part === ".") continue
    if (part === "..") { if (!parts.length) return []; parts.pop() } else if (/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(part)) parts.push(part); else return []
  }
...
function sourceIsSafeForAi(source) { return typeof source === "string" && !SECRET_CONTENT.test(source) }
...
export function selectAiContext(files, request) {
  ...
  while (queued.length) {
    ...
    if (!sourceIsSafeForAi(source)) {
      if (selected.has(file)) fail(422, "Selected AI source contains credential-like content.")
      continue
    }
    total += bytes(source)
    if (total > LIMITS.bytes) fail(422, "Selected AI context exceeds 96 KiB.")
    result.push({ file, source })
    if (result.length >= LIMITS.files) continue
    for (const match of source.matchAll(/(?:from\s*|import\s*)["']([^"']+)["']/g)) for (const candidate of relativeImportCandidates(file, match[1])) if (safeAiPath(candidate) && files.has(candidate) && !queued.includes(candidate) && !result.some(item => item.file === candidate)) queued.push(candidate)
  }
...
export async function runAiRequest({ provider, usage, request, files, userId, projectId, revision, apply }) {
  ...
  try {
    ...
    for (let attempt = 0; attempt <= LIMITS.retries; attempt++) {
      try { proposal = parseProposal(await provider.generate(proposalPrompt(request, context)), new Set(context.map(item => item.file))); break }
    ...
    try { await usage.commit(reservation.id) }
    catch { if (sourceAccepted) throw new AiRequestError(503, "Source was accepted; AI Action settlement is pending."); throw new AiRequestError(503, "AI Action settlement failed.") }
    settled = true
    ...
    // Once canonical source is accepted the reservation is deliberately held
    // for idempotent recovery; releasing it would make the accepted edit free.
