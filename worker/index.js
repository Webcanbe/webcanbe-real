...
import { transitionOperator, transitionSellerApplication, revokeSession, decideSubmissionReview, admitAssessment, promoteAssessmentRelease, publishPromotedListing, grantTestEntitlement, transitionTestEntitlement } from "./control-mutations.js"
                ...
                : path.endsWith("/seller-applications/transition")
                  ...
                  : path.endsWith("/sessions/revoke")
                    ...
                    : path.endsWith("/reviews/decide")
                      ...
                      : path.endsWith("/assessments/admit")
                        ...
                        : path.endsWith("/releases/promote")
                          ...
                          : path.endsWith("/listings/publish")
                            ...
                            : path.endsWith("/entitlements/test/grant")
                              ? await grantTestEntitlement(db, databaseSession, operationBody, proof.evidenceId)
                              : await transitionTestEntitlement(db, databaseSession, operationBody, proof.evidenceId)
              await db.query("COMMIT")
              return json(
                path.endsWith("/operators/transition") ? { operator: result }
                  : path.endsWith("/seller-applications/transition") ? { application: result }
                    : path.endsWith("/sessions/revoke") ? { session: result }
                        ...
                        : path.endsWith("/assessments/admit") ? { assessment: result }
                          : path.endsWith("/releases/promote") ? result
                            : path.endsWith("/listings/publish") ? result
                              : { entitlement: result },
                path.endsWith("/releases/promote") || path.endsWith("/listings/publish") || path.endsWith("/entitlements/test/grant") ? 201 : 200,
              )
            } catch (error) {
              await db.query("ROLLBACK")
              throw error
