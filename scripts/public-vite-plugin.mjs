...
export function publicSitePlugin(){
 const handler=(root,preview)=> (req,res,next)=>{
  ...
  else if(preview&&!path.extname(p)&&!p.startsWith('/__webcanbe/')){if(/^\/(login|signup|dashboard|dashboard-preview|projects|purchases|settings|seller|creators|event|workspace|checkout|auth|_ops|project|marketplace|editor|app\/docs|profile|account|billing|notifications|help)(\/|$)/.test(p))file='/app-shell.html';else{file='/__public/404.html';status=404}}
  if(!file)return next()
