...
const heroBrand=d.querySelector('.wcb-hero nav a[href="/"]');if(heroBrand)heroBrand.classList.add('wcb-hero-brand')
for(const image of d.querySelectorAll('.wcb-hero img[alt="Webcanbe workspace preview"]'))image.alt='' // Decorative imported art, not a product screenshot.
...
// First-party template cards use thumbnails and details from the same source projects as the product catalog.
if(!d.querySelector('.wcb-real-templates')){const section=d.createElement('section');section.className='wcb-real-templates';section.innerHTML='<div class="wcb-real-template-heading"><span class="wcb-section-label">Explore the source projects</span><h2>Two starting points, two distinct projects.</h2><p>Open the full detail and live preview before creating your own editable copy. These are Webcanbe first-party source templates, not seller-published releases.</p></div><div class="wcb-real-template-grid"><article><a href="/project/aperture-north"><img src="/demo/aperture-north/studio-preview.png" alt="Aperture North editorial portfolio preview" loading="lazy"><h3>Aperture North</h3></a><p>An editorial portfolio for design, spaces, and stories. Inspect the long-form layout and adapt its React source.</p><a href="/project/aperture-north/preview">Open live preview →</a></article><article><a href="/project/stillform"><img src="/demo/stillform/hero.jpg" alt="Stillform architecture portfolio preview" loading="lazy"><h3>Stillform</h3></a><p>An architecture portfolio with its own image-led layout and interactions. Create an independent working copy to edit.</p><a href="/project/stillform/preview">Open live preview →</a></article></div>';market.before(section)}
