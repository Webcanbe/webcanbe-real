...
const editor = new HostedEditor(fs.realpathSync(config.applicationRoot), { pool, origins:config.origins,hosts,identityProvider:new OidcIdentityProvider(config.oidc),allowSelfRegistration:config.identity?.allowSelfRegistration === true,sessionLifetimeMs:config.identity?.sessionLifetimeMs,onError:config.localTest === true ? error => console.error("Hosted TEST operation failed:",error) : undefined,fastRefresh:config.fastRefresh === true, staticAssets:config.publicStaticAssets === true ? new TrustedStaticAssets() : undefined, publicRuntimeValueProvider: config.publicRuntimeValues ? new ConfiguredPublicRuntimeValues(config.publicRuntimeValues) : undefined, externalSourceProvider:config.publicGitHubSourceIntake === true ? new GitHubSourceProvider() : undefined })
