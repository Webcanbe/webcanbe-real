...
const server = hostedRunnerGateway({ key: fs.readFileSync(config.key), cert: fs.readFileSync(config.cert), ca: fs.readFileSync(config.ca) }, config.hostId, new PostgresLeaseStore(pool), new LinuxHostRunnerProvider())
