...
for(const file of ['postgres.sql','wcb-gateway.service','gateway-config.example.json','README.md'])fs.copyFileSync(path.join(root,'deployment/hosted',file),path.join(out,file));
