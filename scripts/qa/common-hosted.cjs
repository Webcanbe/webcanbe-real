...
exports.setup=async(options={})=>{
 ...
 const ca=fs.readFileSync(path.join(dir,'cert.pem'),'utf8'),key=fs.readFileSync(path.join(dir,'key.pem'),'utf8'),keys=await generateKeyPair('RS256'),jwk={...await exportJWK(keys.publicKey),kid:'common-test',alg:'RS256'},codes=new Map(),serverErrors=[];let issuer,child,origin;
 ...
 await launch();
 return{root,dir,engine,pool,access,identities,origin,ca,call,api,session,login,launch,shutdown,close,projects,sha,serverErrors};
};
