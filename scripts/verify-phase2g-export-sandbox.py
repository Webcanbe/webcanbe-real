...
profile = repository / 'runtime-profiles' / args.profile
...
# Data-only semver/lock validation from the trusted QA tool. No uploaded module
# or configuration is evaluated here. Vite executes only inside the OS sandbox.
validator = """const fs=require('node:fs'),path=require('node:path'),semver=require(path.join(process.argv[3],'node_modules/semver'));const p=JSON.parse(fs.readFileSync(process.argv[1])),pins=JSON.parse(fs.readFileSync(process.argv[2])).dependencies;for(const[n,r]of Object.entries({...p.dependencies,...p.devDependencies})){if(!pins[n]||!semver.validRange(r)||!semver.satisfies(pins[n],r))throw Error('Unsupported profile declaration: '+n)}"""
subprocess.run([str(Path(shutil.which('node')).resolve()), '-e', validator, str(project / 'package.json'), str(profile / 'package.json'), str(repository)], check=True, capture_output=True)
if lock:
    for name, declared in {**manifest.get('dependencies', {}), **manifest.get('devDependencies', {})}.items():
        entry = lock['packages'].get('node_modules/' + name, {})
        if entry.get('version') != pins.get(name) or entry.get('integrity') != profile_lock.get('node_modules/' + name, {}).get('integrity'):
            raise SystemExit('Export lock differs from pinned profile: ' + name)
...
for location, record in (lock['packages'] if lock else profile_lock).items():
...
(job / 'result.json').write_text(json.dumps({'status': 'PASS', 'archive': str(args.archive.resolve()), 'profile': args.profile, 'lockPresent': lock is not None, 'isolation': json.loads(probe_result), 'dist': str(dist), 'packageScriptsExecuted': False}, indent=2))
print(log)
